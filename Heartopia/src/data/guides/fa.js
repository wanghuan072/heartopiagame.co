export default [
    {
      id: 1,
      title: 'Guide du débutant Heartopia',
      description: 'Un guide complet pour bien débuter sur Heartopia : niveau DG, loisirs, routine quotidienne, astuces de ville, gagner de l’argent et comment débloquer la pêche, le jardinage, la cuisine, l’observation d’oiseaux et les familiers.',
      publishDate: '2026-03-04',
      addressBar: 'beginner-guide',
      imageSrc: '/images/guide/guide01-01.webp',
      imageAlt: 'Couverture du guide du débutant Heartopia',
      seo: {
        title: 'Guide du débutant Heartopia | Guide Heartopia',
        description: 'Guide complet de démarrage pour Heartopia : niveau DG, loisirs, routine quotidienne, conseils de ville et premières étapes.',
        keywords: 'heartopia, débutant, guide, astuces, niveau DG, loisirs, pêche, jardinage',
      },
      detailsHtml: `
          <h2>Bienvenue dans Heartopia</h2>
          <p>Heartopia est une simulation de vie relaxante où vous construisez votre vie sur une île : pêchez, jardinez, cuisinez, liez-vous d'amitié avec les oiseaux et les insectes, et occupez-vous de vos familiers. Ce guide du débutant Heartopia vous accompagne durant vos premiers jours — de l'augmentation de votre niveau DG au déblocage des loisirs, en passant par une routine quotidienne solide et des astuces pour gagner de l'argent tôt dans le jeu.</p>
    
          <h2>Niveau DG &amp; Progression</h2>
          <p>Votre <strong>niveau DG (Guilde de Développement)</strong> est la clé pour débloquer le jeu. Il ouvre de nouveaux loisirs, zones et fonctionnalités. Ne le négligez pas.</p>
          <h3>Comment augmenter son niveau DG</h3>
          <ul>
            <li><strong>Quêtes quotidiennes</strong> — Terminez les 5 quêtes quotidiennes chaque jour pour obtenir des Médailles de Contribution.</li>
            <li><strong>Quêtes hebdomadaires</strong> — Accomplissez les objectifs hebdomadaires pour des récompenses plus importantes.</li>
            <li><strong>Histoire principale</strong> — Progressez dans l'histoire pour gagner des médailles et débloquer des PNJ et des lieux.</li>
          </ul>
          <p>Dépensez vos Médailles de Contribution à la Guilde des membres D.G. pour monter de niveau. Anticipez : certains loisirs ou fonctionnalités nécessitent un niveau DG spécifique (ex: l'Animalerie au Niv.12).</p>
    
          <h2>Vos premiers loisirs</h2>
          <p>Les loisirs sont le cœur de Heartopia. Vous commencez avec quelques-uns et en débloquez d'autres avec des <strong>Tickets d'Extension de Loisir</strong> (obtenus via les récompenses de niveau DG et les quêtes).</p>
          <h3>La Pêche</h3>
          <p>La pêche se débloque tôt dans le tutoriel. Vous attraperez des <a href="/wiki/fish">poissons</a> selon le lieu, la météo et l'heure de la journée. Chaque poisson a une taille d'ombre (P, M, G ou spéciale). Montez de niveau pour attraper des poissons plus rares et gagner plus. Vendez vos poissons ou utilisez-les en <a href="/wiki/recipes">Cuisine</a>.</p>
          <h3>Le Jardinage</h3>
          <p>Le jardinage se débloque également durant le tutoriel. Vous pouvez faire pousser des <strong><a href="/wiki/crops">cultures</a></strong> et des <strong><a href="/wiki/flowers">fleurs</a></strong>. Achetez des graines, plantez, arrosez et récoltez. Un niveau de jardinage plus élevé débloque plus de graines et de meilleurs prix de vente. Consultez le <a href="/wiki">Wiki</a> pour les temps de croissance et les paliers d'étoiles.</p>
          <h3>Débloquer plus de loisirs</h3>
          <p>Utilisez les Tickets d'Extension pour débloquer la <strong>Cuisine</strong> (combinez des ingrédients et des <a href="/wiki/fish">poissons</a> pour des <a href="/wiki/recipes">recettes</a>), l'<strong><a href="/wiki/birds">Observation d'oiseaux</a></strong>, la <strong><a href="/wiki/insects">Capture d'insectes</a></strong>, et plus tard le <strong>Soin des Chats</strong> et <strong>Soin des Chiens</strong> (après avoir atteint le Niv.12 DG et visité l'Animalerie de Mme Joan — voir <a href="/wiki/pets">Familiers</a>). Priorisez selon vos goûts — la cuisine est excellente pour le profit et les quêtes.</p>
    
          <h2>Routine Quotidienne (Que faire chaque jour)</h2>
          <p>Une routine simple vous permet de progresser et de gagner régulièrement.</p>
          <ul>
            <li><strong>Terminer les 5 quêtes quotidiennes</strong> — Priorité absolue pour les Médailles de Contribution et la progression DG.</li>
            <li><strong>Vérifier la planète</strong> — Visitez la planète pour des objets et ressources surprises.</li>
            <li><strong>Arroser les cultures et fleurs</strong> — Ne l'oubliez pas, sinon elles ne pousseront pas à temps.</li>
            <li><strong>Pêcher et cueillir</strong> — Refaites le plein d'énergie et vendez ou cuisinez. Vérifiez la météo pour les <a href="/wiki/fish">poissons</a> rares ; voir <a href="/wiki/foraging">Cueillette</a> pour les points de récolte.</li>
            <li><strong>Parler aux PNJ</strong> — Développez vos relations et déclenchez des quêtes ou l'histoire.</li>
            <li><strong>Visiter les boutiques</strong> — Achetez des graines, des outils et vérifiez les stocks en rotation.</li>
            <li><strong>Cuisiner si débloqué</strong> — Utilisez vos <a href="/wiki/fish">poissons</a> et <a href="/wiki/crops">cultures</a> pour des <a href="/wiki/recipes">recettes</a> ; beaucoup de plats se vendent bien ou sont nécessaires pour les quêtes.</li>
          </ul>
    
          <h2>Ville &amp; PNJ</h2>
          <p>Explorez la ville et parlez à tout le monde. Les PNJ donnent des quêtes, vendent des objets et débloquent des fonctionnalités. Mme Joan gère l' <a href="/wiki/pets">Animalerie</a> (déblocage au Niv.12 DG). D'autres boutiques vendent des graines, des meubles et des objets d'extension. Développer l'amitié avec les personnages débloque souvent de nouveaux dialogues et récompenses.</p>
    
          <h2>Astuces pour gagner de l'argent (Débutants)</h2>
          <ul>
            <li><strong>Vendre les récoltes de haute qualité</strong> — Les cultures et poissons avec plus d'étoiles se vendent plus cher. Concentrez-vous sur la qualité dès que possible.</li>
            <li><strong>Cuisiner et vendre des plats</strong> — De nombreuses <a href="/wiki/recipes">recettes</a> rapportent plus que les ingrédients bruts. Utilisez la <a href="/wiki/recipes">liste des recettes du Wiki</a> pour trouver les plats rentables.</li>
            <li><strong>Faire les quêtes</strong> — L'histoire principale, les quêtes quotidiennes et hebdomadaires rapportent des pièces et des objets.</li>
            <li><strong>Cueillir et vendre</strong> — Les objets de <a href="/wiki/foraging">Cueillette</a> (champignons, herbes, etc.) peuvent être vendus ou utilisés pour l'énergie.</li>
            <li><strong>Ne pas trop dépenser au début</strong> — Économisez pour le réapprovisionnement en graines et les Tickets d'Extension de Loisir.</li>
          </ul>
    
          <h2>Codes Promotionnels</h2>
          <p>Heartopia distribue parfois des codes pour des objets ou de la monnaie. Surveillez les réseaux sociaux officiels ou les avis en jeu. Utilisez la page <a href="/codes"><strong>Codes</strong></a> pour savoir comment les utiliser et trouver les derniers codes Heartopia disponibles.</p>
    
          <h2>Résumé des astuces rapides</h2>
          <ul>
            <li>Faites toujours vos 5 quêtes quotidiennes.</li>
            <li>Montez votre niveau DG et utilisez vos Tickets d'Extension pour la Cuisine et les autres loisirs que vous aimez.</li>
            <li>Arrosez vos plantes tous les jours et planifiez vos récoltes selon le temps de croissance.</li>
            <li>Utilisez le <a href="/wiki">Wiki</a> pour les lieux de <a href="/wiki/fish">pêche</a>, la liste des <a href="/wiki/crops">cultures</a> et les <a href="/wiki/recipes">recettes</a>.</li>
            <li>Parlez aux PNJ et progressez dans l'histoire principale pour débloquer plus de contenu.</li>
          </ul>
          <p>Suivez ce guide du débutant Heartopia et vous prendrez un excellent départ. Pour les <a href="/wiki/fish">poissons</a>, les <a href="/wiki/crops">cultures</a>, les <a href="/wiki/recipes">recettes</a> et les <a href="/wiki/achievements">succès</a>, utilisez le <a href="/wiki">Wiki Heartopia</a> depuis le menu principal.</p>
        `,
    },
    {
      id: 2,
      title: 'Heartopia : quêtes quotidiennes et hebdomadaires',
      description:
        'Comment les quêtes quotidiennes et hebdomadaires et l’histoire principale fournissent des Médailles de Contribution pour monter en niveau DG (Guilde de Développement), avec une checklist pour ne pas prendre de retard.',
      publishDate: '2026-03-23',
      addressBar: 'daily-weekly-quests-dg-medals',
      imageSrc: '/images/guide/guide02-01.webp',
      imageAlt: 'Guide des quêtes quotidiennes et médailles DG Heartopia',
      seo: {
        title: 'Heartopia Quêtes Quotidiennes & Hebdomadaires | Médailles DG & Niveau',
        description:
          'Guide des quêtes Heartopia : terminez 5 quêtes quotidiennes, les objectifs hebdomadaires et l’histoire principale pour des Médailles de Contribution. Dépensez-les à la guilde D.G. pour augmenter le niveau DG.',
        keywords: 'heartopia, quêtes quotidiennes, quêtes hebdomadaires, médailles de contribution, niveau dg, guilde de développement',
      },
      detailsHtml: `
          <h2>Pourquoi les quêtes comptent</h2>
          <p>Votre <strong>niveau DG (Development Guild)</strong> débloque loisirs, zones et systèmes. Les <strong>Médailles de Contribution</strong> se dépensent à la <strong>Guilde des Membres D.G.</strong> pour monter le DG. Les sources les plus fiables sont les <strong>quêtes quotidiennes</strong>, les <strong>quêtes hebdomadaires</strong> et l’<strong>histoire principale</strong>—la même boucle que notre <a href="/guide/beginner-guide">guide du débutant</a>.</p>

          <h2>Quêtes quotidiennes (5 par jour)</h2>
          <p>Heartopia propose <strong>cinq quêtes quotidiennes</strong> chaque jour. Les terminer est l’un des meilleurs moyens d’obtenir des <strong>Médailles de Contribution</strong> pour le DG. Traitez-les en priorité avant l’énergie optionnelle : elles se réinitialisent chaque jour, donc ce que vous sautez attend le cycle suivant.</p>
          <ul>
            <li><strong>Ouvrez le menu des quêtes à la connexion</strong> — Prenez les cinq quêtes et avancez dans un ordre logique (par exemple, combinez les visites aux PNJ en ville avec la pêche ou des courses).</li>
            <li><strong>Alignez-les sur votre routine</strong> — Beaucoup d’objectifs recoupent l’arrosage, la pêche, les dialogues et la cuisine, comme dans le <a href="/guide/beginner-guide">guide du débutant</a>.</li>
          </ul>

          <h2>Quêtes hebdomadaires</h2>
          <p>En plus des quotidiennes, les <strong>objectifs hebdomadaires</strong> offrent en général des <strong>récompenses plus grosses</strong>. Elles prennent plus de temps—étalez-les sur la semaine plutôt que de tout faire au dernier moment. Le progrès hebdomadaire complète le quotidien ; tous deux nourrissent le DG en médailles et butin.</p>

          <h2>Histoire principale</h2>
          <p>L’<strong>histoire principale</strong> accorde des <strong>Médailles de Contribution</strong> et aide à <strong>débloquer PNJ et lieux</strong>. Si le DG stagne, faites avancer l’histoire quand vous pouvez : nouvelles zones et personnages apportent souvent plus de quêtes et de boutiques. L’<strong>animalerie de Mme Joan</strong> s’ouvre au <strong>niveau DG 12</strong>, comme sur la page <a href="/wiki/pets">Familiers</a>.</p>

          <h2>Dépenser les médailles intelligemment</h2>
          <p>À la <strong>Guilde des Membres D.G.</strong>, vous échangez les Médailles de Contribution contre des <strong>montées de niveau DG</strong>. Avant de dépenser, voyez la suite : un DG plus haut débloque des <strong>Tickets d’Extension de Hobby</strong> et des jalons (familiers, loisirs supplémentaires). Monter régulièrement vaut mieux qu’entasser des médailles sans plan.</p>

          <h2>Lien avec les loisirs</h2>
          <p>Les récompenses DG et les quêtes alimentent aussi les <strong>Tickets d’Extension</strong> pour la <strong>Cuisine</strong>, l’<strong><a href="/wiki/birds">observation d’oiseaux</a></strong>, la <strong><a href="/wiki/insects">capture d’insectes</a></strong> et les soins aux familiers. Ensuite, le <a href="/wiki">Wiki</a> liste les <a href="/wiki/fish">poissons</a>, les <a href="/wiki/crops">cultures</a> et les <a href="/wiki/recipes">recettes</a>.</p>

          <h2>Checklist rapide</h2>
          <ul>
            <li>Terminez les <strong>5 quêtes quotidiennes</strong> avant de vous déconnecter.</li>
            <li>Suivez les objectifs <strong>hebdomadaires</strong> tout au long de la semaine.</li>
            <li>Si médailles ou déblocages bloquent, faites progresser l’<strong>histoire principale</strong>.</li>
            <li>Convertissez les médailles à la <strong>Guilde D.G.</strong> pour que le niveau DG continue de monter.</li>
          </ul>

          <h2>Aller plus loin</h2>
          <p>Nouveau sur l’île ? Lisez le <a href="/guide/beginner-guide">guide du débutant Heartopia</a> complet. Pour les codes et récompenses limitées dans le temps : page <a href="/codes">Codes</a>. Les collectionneurs peuvent parcourir les <a href="/wiki/achievements">succès</a> du <a href="/wiki">Wiki Heartopia</a>.</p>
        `,
    },
    {
      id: 3,
      title: 'Comment j’utilise la carte interactive et le wiki (guide joueur)',
      description:
        'Un workflow “joueur” pour planifier ses trajets, suivre ses collectibles et éviter les allers-retours, en combinant la carte interactive, les listes du wiki et la page des codes.',
      publishDate: '2026-03-23',
      addressBar: 'interactive-map-wiki-player-workflow',
      imageSrc: '/images/guide/guide03-01.webp',
      imageAlt: 'Guide joueur : carte interactive et wiki Heartopia',
      seo: {
        title: 'Heartopia Carte interactive & Wiki | Workflow joueur',
        description:
          'Combinez carte interactive et wiki pour planifier des routes, suivre des collectibles et gagner du temps. Checklists pratiques et liens utiles.',
        keywords: 'heartopia, carte interactive, wiki, guide, astuces, itinéraire, collectibles, codes',
      },
      detailsHtml: `
          <h2>Pourquoi je combine Carte + Wiki</h2>
          <p>Quand je joue à Heartopia, j’essaie d’éviter les sessions “au hasard” où je finis par repasser deux fois au même endroit ou oublier ce que je voulais farmer. Ce qui marche le mieux pour moi, c’est de combiner la <a href="/map"><strong>carte interactive</strong></a> avec le <a href="/wiki"><strong>wiki</strong></a>. La carte répond à <em>où</em> aller, et le wiki répond à <em>quoi</em> chercher.</p>

          <h2>Étape 1 : partir d’un objectif (pas d’un lieu)</h2>
          <p>Avant d’ouvrir la carte, je choisis un objectif clair pour la session :</p>
          <ul>
            <li><strong>Compléter une liste</strong> (poissons, insectes, oiseaux, etc.).</li>
            <li><strong>Préparer cuisine/crafting</strong> en récoltant des ingrédients.</li>
            <li><strong>Faire une routine de quêtes</strong> sans faire d’allers-retours.</li>
          </ul>
          <p>Une fois l’objectif défini, je fais une petite checklist via le wiki, puis je trace ma route.</p>

          <h2>Étape 2 : créer une checklist courte avec les listes du wiki</h2>
          <p>Je choisis peu de cibles (souvent 3 à 7). Ce sont les pages que je consulte le plus :</p>
          <ul>
            <li><a href="/wiki/fish"><strong>Poissons</strong></a></li>
            <li><a href="/wiki/insects"><strong>Insectes</strong></a> et <a href="/wiki/birds"><strong>Oiseaux</strong></a></li>
            <li><a href="/wiki/crops"><strong>Cultures</strong></a>, <a href="/wiki/flowers"><strong>Fleurs</strong></a> et <a href="/wiki/foraging"><strong>Cueillette</strong></a></li>
            <li><a href="/wiki/recipes"><strong>Recettes</strong></a></li>
          </ul>
          <p>Je garde volontairement la liste courte : c’est plus simple de terminer un “loop” et de sentir la progression.</p>

          <h2>Étape 3 : planifier l’itinéraire sur la carte interactive</h2>
          <p>Ensuite, j’ouvre la <a href="/map"><strong>carte interactive</strong></a> et je transforme la checklist en itinéraire. Ma règle est simple : <strong>regrouper les objectifs par zone</strong> pour éviter de zigzaguer partout.</p>
          <ul>
            <li><strong>Regrouper les arrêts</strong> — Si je vais dans une zone pour pêcher, j’en profite pour faire la cueillette proche et passer aux shops/PNJ sur le chemin.</li>
            <li><strong>Un loop, un objectif</strong> — Si je farm des ingrédients, je ne mélange pas avec une grosse tournée shopping, sauf si c’est sur la route.</li>
          </ul>

          <h2>Étape 4 : Guides + Codes pour des “gains rapides”</h2>
          <p>Quand je manque de temps, deux pages m’évitent beaucoup d’essais :</p>
          <ul>
            <li><a href="/guide"><strong>Guides</strong></a> — routines et déblocages.</li>
            <li><a href="/codes"><strong>Codes</strong></a> — copier un code cadeau et le redeem en jeu quand des codes sont disponibles.</li>
          </ul>
          <p>Si vous débutez, commencez par le <a href="/guide/beginner-guide">guide du débutant</a>. Si le DG est votre blocage, voyez le <a href="/guide/daily-weekly-quests-dg-medals">guide des quêtes quotidiennes et hebdomadaires</a>.</p>

          <h2>Ma checklist “anti-perte de temps”</h2>
          <ul>
            <li><strong>Choisir un objectif</strong> pour la session.</li>
            <li><strong>Sélectionner 3 à 7 cibles</strong> via le wiki.</li>
            <li><strong>Tracer la route par zones</strong> pour éviter les allers-retours.</li>
            <li><strong>Finir le loop</strong>, puis passer à un autre objectif.</li>
          </ul>

          <h2>Ressources utiles</h2>
          <p>Tout est regroupé dans le <a href="/wiki">Wiki Heartopia</a>. Consultez <a href="/events">Événements</a> pour le contenu limité dans le temps, et utilisez <a href="/characters">Personnages</a> comme page de référence rapide.</p>
        `,
    },
    {
      id: 4,
      title: 'Astuces Heartopia : énergie, cuisine et pêche en mer',
      description:
        'Ne restez pas à sec en pleine route, vendez mieux en cuisinant d’abord, la pêche en mer passe par Bill — moins de tâtonnements.',
      publishDate: '2026-04-07',
      addressBar: 'energy-cooking-sea-fishing-player-tips',
      imageSrc: '/images/guide/guide04-01.webp',
      imageAlt: 'Guide Heartopia énergie cuisine Sea Fishing',
      seo: {
        title: 'Heartopia Énergie, Cuisine et Sea Fishing | Astuces',
        description:
          'Gérez l’énergie avec la nourriture et la cueillette, gagnez de l’or en cuisinant, rejoignez Sea Fishing avec Bill. Poissons, recettes, succès, personnages.',
        keywords: 'heartopia, énergie, cuisine, sea fishing, bill, recettes, poissons, succès, astuces',
      },
      detailsHtml: `
          <h2>Énergie : ce n’est pas que vous “mal routez” — vous manquez de carburant</h2>
          <p>Le classique : mi-parcours, hobbies en attente et <strong>barre vide</strong>. Pêche et cueillette mangent déjà la journée ; les objets de <a href="/wiki/foraging">cueillette</a> remontent la jauge, et avec la <strong>cuisine</strong> les <a href="/wiki/recipes">recettes</a> indiquent l’énergie récupérée — choisissez des plats selon la durée prévue.</p>
          <ul>
            <li><strong>Enchaînez les hobbies avec une pause repas planifiée</strong>, pas en dernier recours, avant les zones lointaines.</li>
            <li><strong>Alignez ingrédients et trajet</strong> — parcourez <a href="/wiki/crops">cultures</a>, <a href="/wiki/foraging">cueillette</a> et <a href="/wiki/fish">poissons</a> pour ne pas bloquer la cuisine pour une herbe.</li>
          </ul>
          <p>Cadre quotidien : <a href="/guide/beginner-guide">guide du débutant</a>.</p>

          <h2>Cuisine : le cru se vend ; le cuit paie souvent les factures</h2>
          <p>L’or lent vient souvent de <strong>tout vendre depuis l’inventaire</strong>. <strong>Les plats prêts battent souvent les piles de matières premières</strong> et beaucoup de quêtes demandent du fourneau. Choisissez <strong>2–3 recettes fixes</strong> dans la <a href="/wiki/recipes">liste des recettes</a> que vous pouvez réellement approvisionner — cuisiner au hasard brûle temps et matériaux.</p>
          <ul>
            <li><strong>Gardez une petite réserve d’ingrédients</strong> pour qu’un daily ne casse pas parce que vous avez vendu la dernière tomate.</li>
          </ul>

          <h2>Sea Fishing : qui organise et qu’est-ce qu’un poisson “événement”</h2>
          <p><strong>Sea Fishing</strong>, c’est bateau / groupe, pas la rive. <strong>Bill</strong> est le <strong>Sea Fishing Host</strong> au <strong>Fishing Village</strong> ; avec lui vous entrez entre amis ou joueurs en ville. Oublié l’emplacement ? La <a href="/characters">liste des personnages</a> le situe.</p>
          <p>Pour la collection : la <a href="/wiki/fish">liste des poissons</a> balise <strong>Sea Fishing</strong> pour ne pas mélanger tables d’eau normales et bassins d’événement. <a href="/wiki/achievements">Succès</a> : <strong>Sea Fishing Master</strong> exige <strong>trois requins issus de golden fish shadows lors d’un seul Sea Fishing Event</strong> — dur mais clair ; rang de pêche plus élevé et priorité aux ombres dorées, c’est ce que vous contrôlez.</p>

          <h2>Avant de quitter le jeu</h2>
          <ul>
            <li>Emportez de la <a href="/wiki/recipes">nourriture</a> si la session dépasse un aller-retour rapide.</li>
            <li>Cuisinez en série quand la boucle farm et la shortlist recettes concordent.</li>
            <li>Jour bateau : trouvez <strong>Bill</strong>, puis croisez les <a href="/wiki/fish">entrées Sea Fishing</a> et l’angle Sea Fishing des <a href="/wiki/achievements">succès</a>.</li>
          </ul>

          <h2>À lire ensuite</h2>
          <p><a href="/guide/daily-weekly-quests-dg-medals">Quêtes quotidiennes &amp; hebdomadaires et médailles DG</a> · <a href="/guide/interactive-map-wiki-player-workflow">Carte + wiki pour les routes</a> · <a href="/codes">Codes</a> · <a href="/events">Événements</a></p>
        `,
    },
    {
      id: 5,
      title: 'Sea Fishing Master : checklist des ombres dorées (Heartopia)',
      description:
        'Si vous ratez les requins ou manquez de temps : checklist Sea Fishing centrée sur les golden fish shadows et l’objectif Sea Fishing Master.',
      publishDate: '2026-04-10',
      addressBar: 'sea-fishing-master-golden-shadows',
      imageSrc: '/images/guide/guide04-01.webp',
      imageAlt: 'Sea Fishing Master checklist ombres dorées',
      seo: {
        title: 'Sea Fishing Master (Heartopia) | Checklist golden fish shadows',
        description:
          'Plan Sea Fishing concret : priorisez les golden fish shadows et visez Sea Fishing Master (3 requins en un seul event).',
        keywords: 'heartopia, sea fishing master, golden fish shadows, requins, sea fishing, bill, succès',
      },
      detailsHtml: `
          <h2>Le problème : « j’ai fait Sea Fishing et j’ai rien eu »</h2>
          <p>Sea Fishing peut vite devenir chaotique : bateau, lancer, remonter… et le timer s’arrête. La solution n’est pas « pêcher plus fort », c’est <strong>choisir la bonne cible</strong>. Dans <a href="/wiki/achievements">Succès</a>, <strong>Sea Fishing Master</strong> est limpide : <strong>trois requins depuis des golden fish shadows lors d’un seul Sea Fishing Event</strong>.</p>

          <h2>Avant de lancer : setup en 60 secondes</h2>
          <ul>
            <li><strong>Repérez le host</strong> — Sea Fishing passe par <strong>Bill</strong> au <strong>Fishing Village</strong>. En cas d’oubli : <a href="/characters">Personnages</a>.</li>
            <li><strong>Mini-plan énergie</strong> — si vous tombez à zéro le jour de l’event, c’est fini. Utilisez <a href="/wiki/recipes">recettes</a> et <a href="/wiki/foraging">cueillette</a> pour recharger.</li>
            <li><strong>Ouvrez vos trackers</strong> — <a href="/wiki/fish">Poissons</a> et <a href="/wiki/achievements">Succès</a>.</li>
          </ul>

          <h2>Pendant l’event : les ombres dorées, point</h2>
          <ul>
            <li><strong>Pas de golden fish shadow = pas prioritaire</strong>.</li>
            <li><strong>Repositionnez vite</strong> — perdre quelques secondes à bouger vaut mieux que griller toute la fenêtre sur la mauvaise ombre.</li>
          </ul>

          <h2>Après : séparez le vrai progrès du “ressenti”</h2>
          <ul>
            <li><strong>Tentative succès</strong> — 3 requins depuis des ombres dorées <em>dans le même event</em> ?</li>
            <li><strong>Collection</strong> — dans <a href="/wiki/fish">Poissons</a>, ciblez les entrées marquées <strong>Sea Fishing</strong>.</li>
            <li><strong>Plafond de skill</strong> — si vous êtes constamment trop lent, montez la pêche hors event puis revenez.</li>
          </ul>

          <h2>Checklist sans excuses</h2>
          <ul>
            <li>Trouvez <strong>Bill</strong> (Fishing Village).</li>
            <li>Emportez de la nourriture (<a href="/wiki/recipes">recettes</a> / <a href="/wiki/foraging">cueillette</a>).</li>
            <li>Dans l’event : <strong>ombres dorées d’abord</strong>.</li>
            <li>Fin : vérifiez <a href="/wiki/achievements">Sea Fishing Master</a> et les entrées <a href="/wiki/fish">Sea Fishing</a>.</li>
          </ul>

          <h2>À lire ensuite</h2>
          <p><a href="/guide/energy-cooking-sea-fishing-player-tips">Énergie + cuisine + Sea Fishing</a> · <a href="/wiki/achievements">Succès</a> · <a href="/wiki/fish">Poissons</a> · <a href="/characters">Personnages</a></p>
        `,
    },
  ]